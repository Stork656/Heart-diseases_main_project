...
def test_df_type_positive(sample_df):
...
def test_check_target_positive(sample_df):
    target = 'a'
    validator.check_target(target, sample_df)


def test_check_target_negative(sample_df):
    target = 'пупупу'
    with pytest.raises(ValueError):
        validator.check_target(target, sample_df)


def test_check_duplicates_positive(sample_df):
    assert validator.check_duplicates(sample_df)


def test_check_duplicates_negative(sample_df):
    no_duplicates_df = sample_df.drop_duplicates()
    assert not validator.check_duplicates(no_duplicates_df)


def test_check_missing_negative(sample_df):
    assert validator.check_missing(sample_df) == False


def test_check_missing_positive(sample_df):
